...
export default function UnitForm({ unit, onChange }: UnitFormProps) {
  ...
  return (
    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200/80 space-y-4" id={`form-container-${unit.id}`}>
      ...
      {/* Barthel Indice */}
      <div className="bg-white p-3 rounded-lg border border-slate-200 space-y-1.5" id="barthel-section">
        ...
        <div className="flex items-center gap-3" id="barthel-slider-container">
          <span className="text-[10px] text-red-500 font-bold">0</span>
          <input
            type="range"
            min="0"
            max="100"
            step="0.01"
            value={unit.barthelMitja}
            onChange={(e) => updateField('barthelMitja', parseFloat(e.target.value) || 0)}
            className="w-full h-1.5 bg-gradient-to-r from-red-400 via-yellow-400 to-emerald-400 rounded-lg appearance-none cursor-pointer accent-slate-800"
            id="slider-barthel"
          />
      ...
      {/* Care Load (Traqueo, VM, Pulsera Errant) */}
